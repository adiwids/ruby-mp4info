...
class MP4Info
  ...
  def initialize(io_stream)
    ...
    if (@info_atoms["SIZE"] && @info_atoms["MS"])
      #
      # Avoid to do BITRATE calculation if all of MM, SS and MS are zero valued.
      # When all MM, SS and MS are zero, BITRATE calculation face division-by-zero case.
      # Set BITRATE to zero.
      if (@info_atoms["MM"] > 0) || (@info_atoms["SS"] > 0) || (@info_atoms["MS"] > 0)
        @info_atoms["BITRATE"] = ( 0.5 + @info_atoms["SIZE"] /
          ( ( @info_atoms["MM"] * 60 + @info_atoms["SS"] + (@info_atoms["MS"] * 1.0) / 1000 ) * 128 ) ).floor
      else
        @info_atoms["BITRATE"] = 0
        @errors.push("Audio duration cannot be zero.")
      end
    end
